export const data = {
  rank: ["E-1", "E-2", "E-4", "E-5", "E-6", "E-7", "E-8", "E-9"],
  organization: [
    "SpOC",
    "SSC",
    "STARCOM",
    "SLD 45",
    "SLD 30",
    "SPD 1",
    "SPD 2",
    "Delta 1",
    "Delta 2",
    "Delta 3",
    "Delta 4",
    "Delta 5",
    "Delta 6",
    "Delta 7",
    "Delta 9",
    "Delta 10",
    "Delat 11",
    "Delta 12",
    "Delta 13",
    "Delta 14",
    "Delta 15",
    "Delta 16",
    "Delta 17",
    "Delta 18",
  ],

  // How to access:
  // data.organization[0].name
  // data.organization[1].squadron
  _organization: [
    { name: "SpOC", squadron: [] },
    { name: "SSC", squadron: [] },
    { name: "STARCOM", squadron: [] },
    { name: "SLD 45", squadron: [] },
    { name: "SLD 30", squadron: [] },
    {
      name: "SPD 1",
      squadron: [
        "21st Communications Squadron",
        "50th Space Communications Squadron",
      ],
    },

    { name: "SPD 2", squadron: [] },
    {
      name: "Delta 1",
      squadron: [
        "1st Delta Operations Squadron",
        "319th Combat Training Squadron",
        "328th Weapons Squadron",
        "392d Combat Training Squadron",
        "533d Training Squadron",
      ],
    },
    {
      name: "Delta 2",
      squadron: [
        "15th Space Surveillance Squadron",
        "18th Space Defense Squadron",
        "19th Space Defense Squadron",
        "20th Space Surveillance Squadron",
      ],
    },

    {
      name: "Delta 3",
      squadron: [
        "3rd Combat Training Squadron",
        "4th Electromagnetic Warfare Squadron",
        "5th Electromagnetic Warfare Squadron",
        "16th Electromagnetic Warfare Squadron",
        "23rd Electromagnetic Warfare Squadron",
        "Intelligence, Surveillance, and Reconnaissance Squadron",
      ],
    },

    {
      name: "Delta 4",
      squadron: [
        "2nd Space Warning Squadron",
        "3rd Satellite Communications Squadron",
        "5th Space Warning Squadron",
        "6th Space Warning Squadron",
        "7th Space Warning Squadron",
        "10th Space Warning Squadron",
        "11th Space Warning Squadron",
        "12th Space Warning Squadron",
        "13th Space Warning Squadron",
      ],
    },

    {
      name: "Delta 5",
      squadron: [
        "55th Combat Training Squadron",
        "Joint Fires and Information Operations Team",
      ],
    },

    {
      name: "Delta 6",
      squadron: [
        "21st Space Operations Squadron",
        "22nd Space Operations Squadron",
        "23rd Space Operations Squadron",
        "62nd Cyberspace Squadron",
        "64th Cyberspace Squadron",
        "65th Cyberspace Squadron",
        "68th Cyberspace Squadron",
        "69th Cyberspace Squadron",
        "645th Cyberspace Squadron",
      ],
    },

    {
      name: "Delta 7",
      squadron: [
        "71st Intelligence, Surveillance, and Reconnaissance Squadron",
        "72nd Intelligence, Surveillance, and Reconnaissance Squadron",
        "73rd Intelligence, Surveillance, and Reconnaissance Squadron",
        "74th Intelligence, Surveillance, and Reconnaissance Squadron",
        "75th Intelligence, Surveillance, and Reconnaissance Squadron",
        "76th Intelligence, Surveillance, and Reconnaissance Squadron",
      ],
    },

    {
      name: "Delta 8",
      squadron: [
        "8th Combat Training Squadron",
        "4th Space Operations Squadron",
        "10th Space Operations Squadron",
        "53rd Satellite Operations Squadron",
      ],
    },

    {
      name: "Delta 9",
      squadron: [
        "9th Combat Training Squadron",
        "1st Space Operations Squadron",
        "3rd Space Operations Squadron",
      ],
    },

    { name: "Delta 10", squadron: ["10th Delta Operations Squadron"] },

    {
      name: "Delta 11",
      squadron: [
        "11th Delta Operations Squadron",
        "25th Space Range Squadron",
        "57th Space Aggressor Squadron",
        "98th Space Range Squadron",
        "527th Space Aggressor Squadron",
      ],
    },

    {
      name: "Delta 12",
      squadron: [
        "12th Delta Operations Squadron",
        "1st Test and Evaluation Squadron",
        "3rd Test and Evaluation Squadron",
        "4th Test and Evaluation Squadron",
        "17th Test and Evaluation Squadron",
      ],
    },

    {
      name: "Delta 13",
      squadron: [
        "13th Delta Operations Squadron",
        "National Security Space Institute",
        "National Security Space Institute",
      ],
    },
    { name: "Delta 14", squadron: [] },
    {
      name: "Delta 15",
      squadron: [
        "15th Command and Control Squadron",
        "15th Cyberspace Squadron",
        "15th Intelligence, Surveillance and Reconnaissance Squadron",
      ],
    },
    { name: "Delta 16", squadron: [] },
    { name: "Delta 17", squadron: [] },
    {
      name: "Delta 18",
      squadron: ["1st Space Analysis Squadron", "2nd Space Analysis Squadron"],
    },
  ],
};
